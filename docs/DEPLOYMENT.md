...
This guide helps organizations clone this repo and deploy it into their own Firebase/GCP project so they retain control (recommended for HIPAA).
		...
		steps:
			...
			- name: Setup Node
				...
				with:
					node-version: '18'
			- run: npm ci
				working-directory: dispatch-app
			- run: npm run build
				working-directory: dispatch-app
...
Notes
- This repo uses `NEXT_PUBLIC_` env vars for Firebase config. Make sure each org fills those from their own Firebase project and never reuses the maintainer's project ids.
...
Need a shorter "quickstart" or a CI/deploy template next? Tell me which and I’ll add it.
