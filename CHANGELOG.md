...
- **Ending an event now actually stops data collection** — a new "End Event" nav item (after Event Summary, on the dispatch page) warns that data collection and logs will stop immediately, then locks the event: every write that goes through the dispatch board (calls, team/supervisor status, equipment) is rejected from that point on, whether the event was ended manually or its designated End Time passed and an hour went by with no dispatch activity logged since (a backup for events nobody remembers to end). A banner appears on the dispatch board once an event is locked, linking to its summary. The venue selection page's event table (and mobile card list) gained a Status column showing "Active" (green) or a clickable "Event Summary" link for an ended event, and clicking an ended event now goes straight to its summary instead of the (now read-only) dispatch board.
