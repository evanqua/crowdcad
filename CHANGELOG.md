...
- **Event-only equipment** — the Equipment step can now add an item that exists only for this event (an "Event only" chip marks it), instead of being limited to checking off items already defined on the venue. Any equipment given a default location now shows a small icon next to that location's marker on the map, the same way it'll appear once the event is live. Equipment cards also got the same sharp corners and lighter background as the Staff Assignments cards, and now use nearly the full width of their column.
