...
- **Event creation is now a 6-step wizard** — Basics, Surge criteria, Teams & supervisors, Equipment, Post schedule, and Review & launch, with the same dot-line-dot progress indicator as venue creation. The venue map preview and the Create Event action move with it: the map stays visible alongside every step, and Create Event now lives on the final Review & launch step. Post schedule now defaults to off (an explicit action turns it on), pre-selects every venue post instead of none, and pre-fills a single posting interval spanning the whole event window instead of pre-splitting it — the ability to split into multiple reposts is unchanged.
